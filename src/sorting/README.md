...
**Сортировка слиянием** - довольно эффективный алгоритм сортировки, хороший пример использования принципа [разделяй и властвуй][divide_et_impera_link].
...
[<h3 id="quick-sort">Quick</h3>](./quick_sort.rs)

![alt text][quick_image]

**Быстрая сортировка, сортировка Хоара** - один из самых быстрых известных универсальных алгоритмов сортировки массивов.

**Quick sort** является существенно улучшенным вариантом алгоритма сортировки с помощью прямого обмена (его варианты известны как [пузырьковая сортировка](#bubble) и [шейкерная сортировка](#cocktail_shaker)), известного в том числе своей низкой эффективностью. Принципиальное отличие состоит в том, что в первую очередь производятся перестановки на наибольшем возможном расстоянии и после каждого прохода элементы делятся на две независимые группы (таким образом улучшение самого неэффективного прямого метода сортировки дало в результате один из наиболее эффективных улучшенных методов).
...
* Сравнить все остальные элементы с опорным и переставить их в массиве так, чтобы разбить массив на три непрерывных отрезка, следующих друг за другом: _элементы меньшие опорного_, _равные_ и _большие_.
* Для отрезков _меньших_ и _больших_ значений выполнить рекурсивно ту же последовательность операций, если длина отрезка больше единицы.
...
**Сортировка Шелла** — алгоритм сортировки, являющийся усовершенствованным вариантом [сортировки вставками](#insertion-sort). Идея метода Шелла состоит в сравнении элементов, стоящих не только рядом, но и на определённом расстоянии друг от друга. Иными словами — это сортировка вставками с предварительными «грубыми» проходами.

Сначала используется сортировка вставками для элементов, стоящие друг от друга на определенном расстоянии - шаге. Затем шаг уменьшается и алгоритм продолжается, а заканчивается он, когда шаг равен 1, то есть после упорядочивания элементов обычной сортировкой вставками.
...
[<h3 id="radix-sort">Radix</h3>](./radix_sort.rs)

![alt text][radix_image]

**Радиальная сортировка** - алгоритм сортировки, который работает только для положительных целых чисел. По сути им можно сортировать все, что можно представить в виде чисел одного размера, но чаще всего алгоритм используется для соритровки чисел и строк.

Алгоритм заключается в том, что сначала элементы массива сортируются по своему младшему разряду, затем на разряд старше, учитывая уже построенный порядок, и так до самого старшего разряда. В итоге массив будет отсортирован. Для соритровки по рязрядам применяется [сортировка подсчетом](#counting-sort), так как мы всегда знаем диапазон значений, которые может принимать разряд.

Сначала мы берем младший разряд и используем подсчет значений. Затем нужно для каждого значения посчитать, сколько есть значений менньших ему. Как уже упоминалось, на каждом этапе сортировки, учитвается предыдущий порядок. Поэтому мы и идем с конца массива, когда в очередной раз сортируем значения. Но чтобы мы знали, на какую позицию необходимо расположить элемент, мы должны знать, сколько есть элементов меньших либо равных ему. Так мы точно определяем нужную позицию, ведь движемся мы с конца.

Здесь, в принципе, не принципиально, по какому основанию мы будем производить расчеты, поэтому ее можно вычилсять динамически, например на основе размера массива, либо же взять определенное значение.

__Time complexity__
* Худшая скорость O((n + b) log_b (k))
* Лучшая скорость O(n)
* Средняя скорость O((n + b) log_b (k))

__Memory__
* O(1)

где `b` -  основание для представления чисел, `k` - максимальное возможное значение.

> **PS:** _Что за сложность такая, и можно ли ее представить иначе?_ Пусть во входных целых числах d цифр. Сортировка по основанию занимает O(d*(n+b)) времени, где b — это основание для представления чисел, например, для десятичной системы b равно 10. Каково значение d? Если k является максимально возможным значением, то d будет равно O(log_b (k)). Таким образом, общая временная сложность составляет O((n + b) * log_b (k)). Что выглядит больше, чем временная сложность алгоритмов сортировки на основе сравнения для большого k. Сначала ограничим k. Пусть k <= n^c , где c — константа. В этом случае сложность становится O((n + b) log_b (n)). Но он все еще не превосходит алгоритмы сортировки на основе сравнения. Что, если мы увеличим значение b? Каким должно быть значение b, чтобы сделать временную сложность линейной? Если мы установим b как n, мы получим временную сложность как O(n). Другими словами, мы можем отсортировать массив целых чисел в диапазоне от 1 до n^c , если числа представлены в системе счисления n (или каждая цифра занимает log_2 (n) бит).
>> **PPS:** _Собственно именно поэтому в алгоритме используется наименьшая возможная степень двойки для представления основания_
...
[quick_image]: https://upload.wikimedia.org/wikipedia/commons/6/6a/Sorting_quicksort_anim.gif "Quick Sort"
[shell_image]: https://upload.wikimedia.org/wikipedia/commons/d/d8/Sorting_shellsort_anim.gif "Shell Sort"
[radix_image]: https://camo.githubusercontent.com/9dfe47a4006039b90addd4df1e419efebc0fdaa105ff25af48d4980d7bb56d04/68747470733a2f2f6473303535757a6574616f62622e636c6f756466726f6e742e6e65742f6272696f6368652f75706c6f6164732f49455a7338784a4d4c332d7261646978736f72745f65642e706e673f77696474683d343030 "Radix Sort"
...
[divide_et_impera_link]: https://ru.wikipedia.org/wiki/%D0%A0%D0%B0%D0%B7%D0%B4%D0%B5%D0%BB%D1%8F%D0%B9_%D0%B8_%D0%B2%D0%BB%D0%B0%D1%81%D1%82%D0%B2%D1%83%D0%B9_(%D0%B8%D0%BD%D1%84%D0%BE%D1%80%D0%BC%D0%B0%D1%82%D0%B8%D0%BA%D0%B0) "Divide et impera link"
